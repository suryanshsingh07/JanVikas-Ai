...
const DepartmentDashboard = () => {
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      const res = await api.get('/submissions?limit=10&sortBy=createdAt&order=desc');
      // api interceptor returns response.data directly, so res = { success, data, pagination }
      const all = res.data || [];
  ...
  const fetchProjects = useCallback(async () => {
    try {
      const res = await api.get('/projects?limit=5&sort=-createdAt');
  ...
  useEffect(() => {
    ...
    loadAll();
  }, [fetchStats, fetchSubmissions, fetchProjects]);
  ...
  const slaBreaches = stats?.submissions?.slaBreaches ?? 0;
  const activeProjects = stats?.projects?.active ?? 0;
  const resolutionRate = stats?.submissions?.resolutionRate ?? 0;
  const officersCount = stats?.citizens?.officers ?? 0;
  const resourceUtil = Math.min(100, Math.max(20, Math.round(activeProjects * 8.5)));
  const interDept = Math.round(totalSub * 0.15);

  const statCards = [
    {
      title: 'Total Submissions', value: totalSub,
      icon: <FileText className="w-5 h-5" />, color: 'blue',
    },
    {
      title: 'Pending Review', value: pendingSub,
      icon: <Clock className="w-5 h-5" />, color: 'orange',
    },
    {
      title: 'SLA Breaches', value: slaBreaches,
      icon: <ShieldAlert className="w-5 h-5" />, color: 'red',
    },
    {
      title: 'Active Projects', value: activeProjects,
      icon: <Briefcase className="w-5 h-5" />, color: 'purple',
    },
    {
      title: 'Resolution Rate', value: resolutionRate, isPercent: true,
      icon: <CheckCircle className="w-5 h-5" />, color: 'green',
    },
    {
      title: 'Resource Utilization', value: resourceUtil, isPercent: true,
      icon: <Activity className="w-5 h-5" />, color: 'blue',
    },
    {
      title: 'Inter-Dept Transfers', value: interDept,
      icon: <Network className="w-5 h-5" />, color: 'purple',
    },
    {
      title: 'Active Personnel', value: officersCount,
      icon: <Users className="w-5 h-5" />, color: 'green',
    },
  ];
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Welcome, <span className="font-semibold text-foreground">{user?.name}</span> · Cross-department issue routing, SLA tracking, and project management.
          </p>
        </div>
            ...
            >
              ...
              <div>
                <div className="text-2xl font-bold">
                  <StatCounter value={s.value} />{s.isPercent ? '%' : ''}
                </div>
        ...
        <div className="lg:col-span-2 glass-card rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {submissionsLoading ? (
              ...
              <div className="p-8 text-center text-gray-500">No active submissions found.</div>
            ) : submissions.map(sub => (
              <Link
                key={sub._id}
                to={`/department/submissions?id=${sub._id}`}
                className="p-4 flex items-start gap-4 hover:bg-surfaceHover/50 transition-colors"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card p-5 rounded-xl">
            ...
            <div className="space-y-2">
              {[
                ...
                { label: 'Notifications', to: '/department/notifications', icon: <Bell size={16} />, color: 'text-sky-500' },
          ...
          <div className="bg-gradient-to-br from-blue-900 to-indigo-900 rounded-xl p-5 text-white shadow-lg relative overflow-hidden">
            ...
            <p className="text-blue-200 text-sm mb-4 relative z-10">
              {totalSub} submissions tracked · {resolutionRate}% resolved · {slaBreaches} SLA breaches
            </p>
            <Link
